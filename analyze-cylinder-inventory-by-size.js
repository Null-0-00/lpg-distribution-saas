...
async function analyzeCylinderInventoryBySize() {
  ...
  try {
    ...
    fullCylindersBySize.forEach((item, index) => {
      ...
      console.log(
        `   Quantity: ${totalQuantity} (${item.quantity} in hand + ${item.quantityWithDrivers} with drivers)`
      );
    ...
    console.log(
      `📊 Total Full Cylinders: ${totalFullCylinders} units = ${totalFullCylindersValue.toFixed(2)} ৳`
    );
    ...
    emptyCylindersBySize.forEach((item, index) => {
      ...
      console.log(`${index + 1}. ${item.cylinderSize.size}:`);
      console.log(
        `   Quantity: ${totalQuantity} (${item.quantityInHand} in hand + ${item.quantityWithDrivers} with drivers)`
      );
      console.log(
        `   Unit Price: ${unitPrice.toFixed(2)} ৳ (20% of full cylinder price)`
      );
      console.log(`   Total Value: ${totalValue.toFixed(2)} ৳`);
      console.log('');
    ...
    console.log(
      `📊 Total Empty Cylinders: ${totalEmptyCylinders} units = ${totalEmptyCylindersValue.toFixed(2)} ৳`
    );
    ...
    if (latestInventory) {
      console.log(
        `Latest Inventory Record (${latestInventory.date.toISOString().split('T')[0]}):`
      );
      ...
      if (totalFullCylinders !== latestInventory.fullCylinders) {
        console.log(
          `⚠️  Full Cylinders MISMATCH: Size-based (${totalFullCylinders}) ≠ Inventory Record (${latestInventory.fullCylinders})`
        );
      } else {
      ...
      if (totalEmptyCylinders !== latestInventory.emptyCylinders) {
        console.log(
          `⚠️  Empty Cylinders MISMATCH: Size-based (${totalEmptyCylinders}) ≠ Inventory Record (${latestInventory.emptyCylinders})`
        );
      } else {
    ...
    console.log('=======================================');
    console.log(
      'Instead of single "Full Cylinders" and "Empty Cylinders" assets,'
    );
    ...
    fullCylindersBySize.forEach((item) => {
      ...
      if (totalQuantity > 0) {
        console.log(
          `• Full Cylinders (${item.cylinderSize.size}): ${totalQuantity} × ${unitPrice.toFixed(2)} ৳ = ${totalValue.toFixed(2)} ৳`
        );
      }
    });
    ...
    emptyCylindersBySize.forEach((item) => {
      ...
      if (totalQuantity > 0) {
        console.log(
          `• Empty Cylinders (${item.cylinderSize.size}): ${totalQuantity} × ${unitPrice.toFixed(2)} ৳ = ${totalValue.toFixed(2)} ৳`
        );
      }
    });
