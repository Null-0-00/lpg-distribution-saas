...
async function checkActualEmptyCylinderBreakdown() {
  ...
  try {
    ...
    console.log(`   Date: ${latestInventory?.date.toISOString().split('T')[0]}`);
    console.log(`   Total Empty Cylinders: ${latestInventory?.emptyCylinders || 0}`);
    console.log(`   Total Full Cylinders: ${latestInventory?.fullCylinders || 0}`);
    console.log('');
    ...
    activeDriversWithReceivables.forEach((driver, index) => {
      const receivables = driver.receivableRecords[0]?.totalCylinderReceivables || 0;
    ...
    if (driverIds.length > 0) {
      ...
      allBaselineBreakdowns.forEach((baseline, index) => {
        console.log(`   ${index + 1}. ${baseline.driver.name} - ${baseline.cylinderSize.size}: ${baseline.baselineQuantity} cylinders`);
      });
      console.log('');
      ...
      for (const driver of activeDriversWithReceivables) {
        ...
        if (latestRecord?.totalCylinderReceivables > 0) {
          ...
          salesWithCylinders.forEach((sale) => {
            const size = sale.product?.cylinderSize?.size || sale.product?.size || sale.product?.name || 'Unknown';
            const receivablesChange = (sale.quantity || 0) - (sale.cylindersDeposited || 0);
            
            sizeBreakdown[size] = (sizeBreakdown[size] || 0) + receivablesChange;
            console.log(`      ${size}: ${receivablesChange > 0 ? '+' : ''}${receivablesChange} (sale: ${sale.quantity}, deposited: ${sale.cylindersDeposited})`);
          });
    ...
    cylinderSizes.forEach((cylinderSize) => {
      ...
      if (totalReceivables > 0 && receivablesForSize > 0) {
        ...
        console.log(`${size}: ${receivablesForSize}/${totalReceivables} = ${(proportion * 100).toFixed(1)}% → ${emptyCylindersForSize} empty cylinders`);
      } else if (cylinderSizes.length > 0) {
        emptyCylindersForSize = Math.floor(totalEmptyCylinders / cylinderSizes.length);
        console.log(`${size}: Equal distribution → ${emptyCylindersForSize} empty cylinders`);
      }
      
      const emptyCylindersInHand = Math.max(0, emptyCylindersForSize - receivablesForSize);
      console.log(`   In Hand: ${emptyCylindersInHand}, With Drivers: ${receivablesForSize}`);
    });
    ...
    console.log('');
    console.log('If this is correct, the calculation logic needs to be updated');
    console.log('to use the actual size-specific data instead of proportional distribution.');
