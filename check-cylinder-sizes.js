...
async function checkCylinderSizes() {
  try {
    ...
    // Check available cylinder sizes in the system
    ...
    const cylinderSizes = await prisma.cylinderSize.findMany({
      ...
      select: { size: true, description: true },
    });

    cylinderSizes.forEach((size) => {
      console.log(
        `- ${size.size} ${size.description ? `(${size.description})` : ''}`
      );
    });
