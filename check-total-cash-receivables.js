...
async function checkTotalCashReceivables() {
  ...
  try {
    ...
    latestRecords.forEach((record, index) => {
      ...
      console.log(
        `      Status: ${record.driver.status}, Type: ${record.driver.driverType}`
      );
      console.log('');
    ...
    console.log(
      `   Total Cash Receivables: ${totalCashReceivables.toFixed(2)} ৳`
    );
    console.log(
      `   Total Cylinder Receivables: ${totalCylinderReceivables} units`
    );
    console.log(
      `   Combined Total (OLD): ${(totalCashReceivables + totalCylinderReceivables).toFixed(2)} ৳`
    );
    console.log(
      `   Dashboard Should Show (NEW): ${totalCashReceivables.toFixed(2)} ৳ (cash only) ✅\n`
    );
    ...
    if (dashboardQuery) {
      ...
      console.log(
        `   Uses single latest record: ${dashboardQuery.totalCashReceivables.toFixed(2)} ৳ cash`
      );
      console.log(`   This might not include all drivers!`);
      console.log('');
      console.log('💡 Recommendation:');
      console.log(
        '   Dashboard should sum cash receivables from all active retail drivers'
      );
