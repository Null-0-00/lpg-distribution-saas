...
if (missingKeys.length > 0) {
  console.log('Missing keys:', missingKeys.slice(0, 10).join(', '), missingKeys.length > 10 ? '...' : '');
  
  // For now, let's just add placeholder translations for missing keys
  let updatedContent = content;
  
  // Find the end of Bengali translations object
  const bengaliEndMatch = content.match(/(const bengaliTranslations: Translations = \{[\s\S]*?)\n\};/);
  if (bengaliEndMatch) {
