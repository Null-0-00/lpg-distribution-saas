...
export async function GET(request: NextRequest) {
  try {
    ...
    const [assignments, totalCount, distributors, companyTiers, productTiers] = await Promise.all([
    ...
    return NextResponse.json(
      createAdminErrorResponse(error instanceof Error ? error.message : 'Failed to fetch pricing assignments'),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!companyTierId && !productTierId) {
      return NextResponse.json(
        createAdminErrorResponse('Either company tier or product tier must be specified'),
        { status: 400 }
      );
    }
    ...
    const conflictingAssignment = await prisma.distributorPricingAssignment.findFirst({
      where: {
        tenantId,
    ...
    if (conflictingAssignment) {
      return NextResponse.json(
        createAdminErrorResponse('A similar active pricing assignment already exists for this distributor'),
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      createAdminResponse(assignment, 'Pricing assignment created successfully'),
      { status: 201 }
    );
  } catch (error) {
    console.error('Create pricing assignment error:', error);
    return NextResponse.json(
      createAdminErrorResponse(error instanceof Error ? error.message : 'Failed to create pricing assignment'),
      { status: error instanceof Error && error.message.includes('Admin') ? 403 : 500 }
    );
  }
}