...
export async function GET(request: NextRequest) {
  try {
    ...
    const aggregations = await cylinderTrackingService.getCylinderTrackingAggregation(query);

    // Get detailed records if specific filters are applied
    let detailedRecords = null;
    if (queryParams.cylinderSizeId || queryParams.productId || queryParams.eventId) {
      const whereClause: any = {
        tenantId,
        ...(queryParams.cylinderSizeId && { cylinderSizeId: queryParams.cylinderSizeId }),
    ...
    const summary = {
      ...
      totalOnboardingFullCylinders: aggregations.reduce((sum, agg) => sum + agg.totalOnboardingFullCylinders, 0),
      totalOnboardingEmptyCylinders: aggregations.reduce((sum, agg) => sum + agg.totalOnboardingEmptyCylinders, 0),
      totalOnboardingCylinderReceivables: aggregations.reduce((sum, agg) => sum + agg.totalOnboardingCylinderReceivables, 0),
      totalPackageSales: aggregations.reduce((sum, agg) => sum + agg.totalPackageSales, 0),
      totalRefillSales: aggregations.reduce((sum, agg) => sum + agg.totalRefillSales, 0),
      totalSalesRevenue: aggregations.reduce((sum, agg) => sum + agg.totalSalesRevenue, 0),
      totalIncomingFull: aggregations.reduce((sum, agg) => sum + agg.totalIncomingFull, 0),
      totalIncomingEmpty: aggregations.reduce((sum, agg) => sum + agg.totalIncomingEmpty, 0),
      totalShipmentCost: aggregations.reduce((sum, agg) => sum + agg.totalShipmentCost, 0),
      currentTotalFullCylinders: aggregations.reduce((sum, agg) => sum + agg.currentFullCylinders, 0),
      currentTotalEmptyCylinders: aggregations.reduce((sum, agg) => sum + agg.currentEmptyCylinders, 0),
      currentTotalCylinderReceivables: aggregations.reduce((sum, agg) => sum + agg.currentCylinderReceivables, 0),
      currentTotalCashReceivables: aggregations.reduce((sum, agg) => sum + agg.currentCashReceivables, 0)
    };
    ...
    switch (action) {
      case 'recalculate': {
        ...
        if (!cylinderSizeId || !dateFrom || !dateTo) {
          return NextResponse.json(
            { error: 'cylinderSizeId, dateFrom, and dateTo are required for recalculation' },
            { status: 400 }
          );
        }
      ...
      case 'export': {
        ...
        if (format === 'csv') {
          // Return CSV format
          const csvHeader = 'Cylinder Size,Current Full,Current Empty,Total Receivables,Total Sales Revenue,Total Shipment Cost\n';
          const csvData = aggregations.map(agg => 
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid action. Supported actions: recalculate, audit, export' },
          { status: 400 }
        );
    }
