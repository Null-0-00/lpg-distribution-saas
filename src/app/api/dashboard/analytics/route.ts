...
export async function GET(request: NextRequest) {
  try {
      ...
      topDriversToday
        ...
        .map(async (driverData) => {
          ...
          const maxSales = Math.max(
            ...topDriversToday.map((d) => d._sum.quantity || 0)
          );
          const percentage =
            maxSales > 0
              ? Math.round(((driverData._sum.quantity || 0) / maxSales) * 100)
              : 0;
    ...
    const [inventoryRecords, receivableRecords, recentSales] =
      await Promise.all([
    ...
    return NextResponse.json({
      ...
      performanceMetrics: {
        salesTrend:
          (weekSalesData[6]?.quantity ?? 0) > (weekSalesData[5]?.quantity ?? 0)
            ? 'up'
            : 'down',
        totalWeekSales: weekSalesData.reduce(
          (sum, day) => sum + day.quantity,
          0
        ),
        avgDailySales:
          weekSalesData.reduce((sum, day) => sum + day.quantity, 0) / 7,
      },
    });
  } catch (error) {
