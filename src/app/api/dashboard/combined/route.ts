...
export async function GET(request: NextRequest) {
  try {
    ...
    const topDrivers = topDriversData.length > 0 ? await Promise.resolve(topDriversData).then(async (results) => {
      ...
      return results.map(driverSales => {
        const driverInfo = drivers.find(d => d.id === driverSales.driverId);
        return {
          name: driverInfo?.name || 'Unknown Driver',
          sales: driverSales._count.id,
          revenue: driverSales._sum.netValue || 0,
          percentage: Math.round((driverSales._count.id / maxSales) * 100)
        };
      });
    }) : [];
    ...
    const weekSales = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayData = weekSalesData.find(d => 
        new Date(d.saleDate).toDateString() === date.toDateString()
      );
