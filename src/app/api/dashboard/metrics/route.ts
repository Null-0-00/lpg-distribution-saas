...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get today's sales
    const [todaysSales, totalRevenue, pendingReceivables, lowStockAlerts, pendingApprovals, monthlyExpenses] = await Promise.all([
      // Today's sales count
      prisma.sale.count({
        where: {
          tenantId,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Dashboard preferences updated for user:', session.user.id, preferences);
