...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!babluRecord) {
      return NextResponse.json({ error: 'Bablu record not found' }, { status: 404 });
    }
    ...
    const correctTotal = babluRecord.cashReceivablesChange + (babluRecord.onboardingCashReceivables || 0) + 0; // previousTotal = 0 for same day
