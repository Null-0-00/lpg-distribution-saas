...
export async function POST(request: NextRequest) {
  try {
    console.log(`🔄 Starting receivables recalculation for ALL tenants (DEBUG MODE)`);
    ...
    const recordsByTenantAndDriver = new Map<string, Map<string, typeof allRecords>>();
    ...
    for (const [tenantId, driverRecords] of recordsByTenantAndDriver.entries()) {
      console.log(`🏢 Processing tenant ${tenantId} with ${driverRecords.size} drivers`);
      
      // Process each driver's records in chronological order
      for (const [driverId, records] of driverRecords.entries()) {
        console.log(`👤 Processing driver ${driverId} with ${records.length} records`);
