...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `✅ TESTING: Completed receivables recalculation in ${duration.toFixed(2)}ms`
    );
    ...
    return NextResponse.json({
      ...
      performance: {
        ...
        daysProcessed: days,
        optimization:
          'Batch queries with exact calculations + onboarding support',
      },
    });
  } catch (error) {
...
async function calculateReceivablesOptimized(tenantId: string, days: number) {
  ...
  for (const date of dates) {
    ...
    for (const driver of drivers) {
      ...
      console.log(`${driver.name} calculation:`, {
        ...
        needsUpdate: todaysRecord
          ? Math.abs(totalCashReceivables - todaysRecord.totalCashReceivables) >
            0.01
          : true,
      });
  ...
  console.log(
    `💾 Would execute ${upsertOperations.length} upsert operations...`
  );
