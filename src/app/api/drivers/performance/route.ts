...
export async function GET(request: NextRequest) {
  try {
    ...
    const performanceData = drivers.map(driver => {
      ...
      const dailySalesTotal = driver.dailySales.reduce((acc, daily) => ({
      ...
      if (dailySalesTotal.totalSales === 0 && driver.sales.length > 0) {
        packageSalesCount = driver.sales.filter(sale => sale.saleType === 'PACKAGE').reduce((sum, sale) => sum + sale.quantity, 0);
        refillSalesCount = driver.sales.filter(sale => sale.saleType === 'REFILL').reduce((sum, sale) => sum + sale.quantity, 0);
        
        // Calculate revenue from individual sales
        dailySalesTotal.totalRevenue = driver.sales.reduce((sum, sale) => sum + sale.totalValue, 0);
        dailySalesTotal.netRevenue = driver.sales.reduce((sum, sale) => sum + sale.netValue, 0);
      }
    ...
    const summary = {
      ...
      totalPackageSales: performanceData.reduce((sum, d) => sum + d.totalPackageSales, 0),
      totalRefillSales: performanceData.reduce((sum, d) => sum + d.totalRefillSales, 0),
      totalCashReceivables: performanceData.reduce((sum, d) => sum + d.totalCashReceivables, 0),
      totalCylinderReceivables: performanceData.reduce((sum, d) => sum + d.totalCylinderReceivables, 0),
      totalRevenue: performanceData.reduce((sum, d) => sum + d.totalRevenue, 0),
      averagePackageSales: performanceData.length > 0 ? performanceData.reduce((sum, d) => sum + d.totalPackageSales, 0) / performanceData.length : 0,
      averageRefillSales: performanceData.length > 0 ? performanceData.reduce((sum, d) => sum + d.totalRefillSales, 0) / performanceData.length : 0
    };
