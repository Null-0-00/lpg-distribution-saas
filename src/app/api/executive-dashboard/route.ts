...
async function getSalesMetrics(tenantId: string, dateRange: { startDate: Date; endDate: Date }) {
  ...
  const previousRevenue = previousPeriodSales.reduce((sum, sale) => sum + sale.netValue, 0);
...
async function getInventoryMetrics(tenantId: string) {
  ...
  const inventoryValue = products.reduce((sum, product) => 
    sum + (product.currentPrice * (currentInventory?.fullCylinders || 0)), 0
  );

  const turnoverRate = currentInventory ? 
    (currentInventory.totalSales / Math.max(currentInventory.fullCylinders, 1)) * 30 : 0;
...
async function getReceivablesMetrics(tenantId: string, dateRange: { startDate: Date; endDate: Date }) {
  const { startDate, endDate } = dateRange;
  ...
  const totalCashReceivables = receivables.reduce((sum, r) => sum + r.totalCashReceivables, 0);
  const totalCylinderReceivables = receivables.reduce((sum, r) => sum + r.totalCylinderReceivables, 0);
  
  const overdueAmount = salesOnCredit
    .filter(sale => {
      const daysDiff = (new Date().getTime() - sale.saleDate.getTime()) / (1000 * 60 * 60 * 24);
      return daysDiff > 30;
    })
    .reduce((sum, sale) => sum + sale.netValue, 0);

  const collectionRate = totalCashReceivables > 0 ? 
    ((totalCashReceivables - overdueAmount) / totalCashReceivables) * 100 : 100;
...
async function getDriverPerformance(tenantId: string, dateRange: { startDate: Date; endDate: Date }) {
  const { startDate, endDate } = dateRange;
  ...
  const driverStats = drivers.map(driver => {
    ...
    return {
      ...
      averageOrderValue: driverSales.length > 0 ? revenue / driverSales.length : 0,
  ...
  const averagePerformance = driverStats.reduce((sum, d) => sum + d.performance, 0) / Math.max(driverStats.length, 1);
...
async function getFinancialHealth(tenantId: string, dateRange: { startDate: Date; endDate: Date }) {
  const { startDate, endDate } = dateRange;
  ...
  const currentRatio = totalLiabilities > 0 ? totalAssets / totalLiabilities : totalAssets > 0 ? 999 : 1;
  ...
  return {
    ...
    healthScore: Math.min(100, Math.max(0, 
      ...
      (returnOnAssets * 0.3) + 30
    ))
  };
}

async function getTrendsData(tenantId: string, dateRange: { startDate: Date; endDate: Date }) {
  ...
  return {
    sales: Object.entries(dailyTrends).map(([date, data]) => ({
      date,
      ...data
    })).sort((a, b) => a.date.localeCompare(b.date)),
    expenses: Object.entries(expenseTrends).map(([date, amount]) => ({
      date,
      amount
    })).sort((a, b) => a.date.localeCompare(b.date))
  };
