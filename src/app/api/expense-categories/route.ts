...
export async function GET(request: NextRequest) {
  try {
    ...
    const targetMonth = monthParam ? parseInt(monthParam) : currentDate.getMonth() + 1; // 1-based month
    ...
    const categoriesWithBudgetInfo = categories.map(category => {
      const currentMonthSpending = category.expenses.reduce((sum, expense) => sum + expense.amount, 0);
    ...
    const totalSpending = categoriesWithBudgetInfo.reduce((sum, cat) => sum + cat.currentMonthSpending, 0);
    const overBudgetCategories = categoriesWithBudgetInfo.filter(cat => cat.isOverBudget).length;
