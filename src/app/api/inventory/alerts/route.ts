...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const product of products) {
      ...
      if (currentLevels.fullCylinders === 0) {
        alerts.push({
          ...
          recommendedAction: 'Place urgent purchase order or transfer stock from other locations',
          createdAt: new Date()
        });
      }
      ...
      const adjustmentMovements = recentMovements.filter(m => 
      ...
      if (adjustmentMovements.length > 3) {
        alerts.push({
          ...
          currentStock: currentLevels.fullCylinders,
          recommendedAction: 'Review inventory processes and investigate potential discrepancies',
          createdAt: new Date()
        });
      }
      ...
      if (currentLevels.fullCylinders > overstockThreshold) {
        alerts.push({
          ...
          recommendedAction: 'Consider reducing future orders or transferring stock to other locations',
          createdAt: new Date()
        });
      }
    }
    ...
    const summary = {
      ...
      byType: {
        outOfStock: filteredAlerts.filter(a => a.type === 'OUT_OF_STOCK').length,
        lowStock: filteredAlerts.filter(a => a.type === 'LOW_STOCK').length,
        overstock: filteredAlerts.filter(a => a.type === 'OVERSTOCK').length,
        anomalies: filteredAlerts.filter(a => a.type === 'MOVEMENT_ANOMALY').length
      }
    };
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!productId || !alertType) {
      return NextResponse.json({ error: 'Product ID and alert type required' }, { status: 400 });
    }
    ...
    const inventoryCalculator = new InventoryCalculator(prisma);
    const currentLevels = await inventoryCalculator.getCurrentInventoryLevels(tenantId, productId);
    ...
    const daysOfStockRemaining = avgDailySales > 0 ? Math.floor(currentLevels.fullCylinders / avgDailySales) : Infinity;
    ...
    return NextResponse.json({
      ...
      analytics: {
        ...
        daysOfStockRemaining: daysOfStockRemaining === Infinity ? 'N/A' : daysOfStockRemaining,
