...
export async function GET(request: NextRequest) {
  try {
    ...
    const categorizedLiabilities = {
      CURRENT: liabilities.filter(
        (l) => l.category === LiabilityCategory.CURRENT_LIABILITY
      ),
      LONG_TERM: liabilities.filter(
        (l) => l.category === LiabilityCategory.LONG_TERM_LIABILITY
      ),
    };

    // Calculate totals
    const totals = {
      CURRENT: categorizedLiabilities.CURRENT.reduce(
        (sum, l) => sum + l.amount,
        0
      ),
      LONG_TERM: categorizedLiabilities.LONG_TERM.reduce(
        (sum, l) => sum + l.amount,
        0
      ),
...
async function calculateOwnerEquity(tenantId: string, asOfDate: Date) {
  try {
    ...
    const totalAutoAssets = autoAssets.reduce(
    ...
    const netIncome =
      (totalRevenue._sum.totalValue || 0) - (totalExpenses._sum.amount || 0);
    ...
    if (cylinderReceivables._sum.totalCylinderReceivables) {
      ...
      if (recentProduct) {
        assets.push({
          ...
          currentValue:
            cylinderReceivables._sum.totalCylinderReceivables *
