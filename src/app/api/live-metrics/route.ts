...
async function getRealTimeStats(tenantId: string) {
  ...
  const [recent15MinSales, recentHourSales, currentInventory] = await Promise.all([
    prisma.sale.findMany({
      where: {
        tenantId,
  ...
  const revenue15Min = recent15MinSales.reduce((sum, sale) => sum + sale.netValue, 0);
  const revenueHour = recentHourSales.reduce((sum, sale) => sum + sale.netValue, 0);
...
async function getActiveDriverStats(tenantId: string, todayStart: Date) {
  ...
  const driverStats = drivers.map(driver => {
    const driverSales = todaySales.filter(sale => sale.driverId === driver.id);
    ...
    return {
      ...
      revenue,
      quantity,
      lastSale: driverSales.length > 0 ? 
        Math.max(...driverSales.map(s => s.createdAt.getTime())) : null
    };
  });
...
function calculateGrowth(today: any, yesterday: any) {
  ...
  return {
    ...
    avgOrderValue: calculatePercentChange(today.avgOrderValue, yesterday.avgOrderValue),
