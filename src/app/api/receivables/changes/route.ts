  ...
  try {
    ...
    const processedChanges = changes.map(change => {
      ...
      if (newValues) {
        ...
        customerName = newValues.customerName || metadata?.customerName || 'Unknown Customer';
      } else if (oldValues) {
        receivableType = oldValues.receivableType || 'UNKNOWN';
        amount = oldValues.amount || 0;
        quantity = oldValues.quantity || 0;
        customerName = oldValues.customerName || metadata?.customerName || 'Unknown Customer';
      }
      ...
      if (change.action === 'UPDATE') {
        if (oldValues && newValues) {
          ...
          } else if (oldValues.status !== newValues.status && newValues.status === 'PAID') {
