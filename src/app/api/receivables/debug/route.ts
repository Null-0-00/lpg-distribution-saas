...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!driverName) {
      return NextResponse.json({ error: 'Driver name required' }, { status: 400 });
    }
    ...
    console.log(`Debugging receivables for driver: ${driver.name} (${driver.id})`);
    ...
    const salesTotals = recentSales.reduce((acc, sale) => {
      const cashReceivable = sale.totalValue - sale.discount - sale.cashDeposited;
      const cylinderReceivable = sale.saleType === 'REFILL' ? sale.quantity - sale.cylindersDeposited : 0;
      ...
      return {
        ...
        totalCylinderDeposited: acc.totalCylinderDeposited + sale.cylindersDeposited,
        calculatedCashReceivables: acc.calculatedCashReceivables + Math.max(0, cashReceivable),
        calculatedCylinderReceivables: acc.calculatedCylinderReceivables + Math.max(0, cylinderReceivable),
        refillSalesQty: acc.refillSalesQty + (sale.saleType === 'REFILL' ? sale.quantity : 0)
      };
    }, {
      totalSalesValue: 0,
