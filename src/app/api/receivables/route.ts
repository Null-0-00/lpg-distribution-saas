...
export async function GET(request: NextRequest) {
  try {
    ...
    const { driverId, startDate, endDate, page, limit } = receivablesQuerySchema.parse(
    ...
    const summary = driverSummary.map(item => ({
      ...
      totalReceivables: (item._sum.totalCashReceivables || 0) + (item._sum.totalCylinderReceivables || 0)
    })).sort((a, b) => b.totalReceivables - a.totalReceivables);

    // Overall totals
    const overallTotals = {
      totalCashReceivables: summary.reduce((sum, s) => sum + s.currentCashReceivables, 0),
      totalCylinderReceivables: summary.reduce((sum, s) => sum + s.currentCylinderReceivables, 0),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await calculateDailyReceivables(tenantId, targetDate, driverId);
...
async function calculateDailyReceivables(tenantId: string, date: Date, driverId?: string) {
