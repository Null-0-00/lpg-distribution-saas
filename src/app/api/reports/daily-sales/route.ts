...
export async function GET(request: NextRequest) {
  try {
    ...
    const previousReceivablesMap = new Map(previousReceivables.map(r => [r.driverId, r]));
    ...
    const driverReports = allDrivers.map(driver => {
      ...
      const totalSalesValue = driverSales.reduce((sum, s) => sum + s.totalValue, 0);
      const discount = driverSales.reduce((sum, s) => sum + s.discount, 0);
      const cashDeposited = driverSales.reduce((sum, s) => sum + s.cashDeposited, 0);
      const cylinderDeposited = driverSales.reduce((sum, s) => sum + s.cylindersDeposited, 0);

      // EXACT FORMULAS as specified:
      // Today's Changes in Drivers Cash Receivables = driver sales revenue - cash deposits - discounts
      const todaysCashReceivableChange = totalSalesValue - cashDeposited - discount;
      ...
      const yesterdaysCashReceivables = previousReceivables?.totalCashReceivables || 0;
      const yesterdaysCylinderReceivables = previousReceivables?.totalCylinderReceivables || 0;

      // EXACT FORMULAS as specified:
      // Today's Total cash receivable = Yesterday's Total + Today's Changes
      const todaysTotalCashReceivables = yesterdaysCashReceivables + todaysCashReceivableChange;
      
      // Today's Total cylinder receivable = Yesterday's Total + Today's Changes
      const todaysTotalCylinderReceivables = yesterdaysCylinderReceivables + todaysCylinderReceivableChange;

      // Total receivables (cash + cylinder)
      const totalReceivables = todaysTotalCashReceivables + todaysTotalCylinderReceivables;

      // Calculate total change in receivables
      const previousTotalReceivables = yesterdaysCashReceivables + yesterdaysCylinderReceivables;
    ...
    const totals = {
      ...
      totalSalesValue: driverReports.reduce((sum, d) => sum + d.totalSalesValue, 0),
      discount: driverReports.reduce((sum, d) => sum + d.discount, 0),
      totalDeposited: driverReports.reduce((sum, d) => sum + d.totalDeposited, 0),
      totalCashReceivables: driverReports.reduce((sum, d) => sum + d.totalCashReceivables, 0),
      totalCylinderReceivables: driverReports.reduce((sum, d) => sum + d.totalCylinderReceivables, 0),
      totalReceivables: driverReports.reduce((sum, d) => sum + d.totalReceivables, 0),
      changeInReceivables: driverReports.reduce((sum, d) => sum + d.changeInReceivables, 0),
      changeInCashReceivables: driverReports.reduce((sum, d) => sum + d.changeInCashReceivables, 0),
      changeInCylinderReceivables: driverReports.reduce((sum, d) => sum + d.changeInCylinderReceivables, 0)
    };
    ...
    const totalDriverCashDeposits = driverCashDeposits.reduce((sum, d) => sum + (d._sum.cashDeposited || 0), 0);
...
async function calculateDailyReceivablesForDate(tenantId: string, date: Date) {
  ...
  for (const driver of drivers) {
    ...
    const salesData = await prisma.sale.aggregate({
      where: {
        ...
        saleDate: {
          ...
          lt: new Date(new Date(date.getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T00:00:00.000Z')
        }
      },
      _sum: {
        totalValue: true,
    ...
    const totalCylinderReceivables = yesterdayCylinderTotal + cylinderReceivablesChange;
