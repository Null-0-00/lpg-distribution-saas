...
export async function PUT(
  ...
  try {
    ...
    const permissionCheck = BusinessValidator.validateUserPermission(role, 'UPDATE_SALE');
    ...
    if (validatedData.quantity !== undefined || validatedData.unitPrice !== undefined || 
        validatedData.discount !== undefined || validatedData.paymentType !== undefined ||
      ...
      const businessValidation = BusinessValidator.validateSale({
        ...
        cashDeposited: validatedData.cashDeposited ?? existingSale.cashDeposited,
        netValue,
        cylindersDeposited: validatedData.cylindersDeposited ?? existingSale.cylindersDeposited,
        isOnCredit: (validatedData.cashDeposited ?? existingSale.cashDeposited) < netValue,
      });
    ...
    if (validatedData.quantity && validatedData.quantity > existingSale.quantity) {
    ...
    const updatedSale = await prisma.$transaction(async (tx) => {
      const sale = await tx.sale.update({
        ...
        data: {
          ...
          ...(validatedData.paymentType && { paymentType: validatedData.paymentType }),
          ...(validatedData.cashDeposited !== undefined && { cashDeposited: validatedData.cashDeposited }),
          ...(validatedData.cylindersDeposited !== undefined && { cylindersDeposited: validatedData.cylindersDeposited }),
          ...(validatedData.notes !== undefined && { notes: validatedData.notes }),
          totalValue,
          netValue,
          isOnCredit: (validatedData.cashDeposited ?? existingSale.cashDeposited) < netValue,
          isCylinderCredit: (validatedData.saleType ?? existingSale.saleType) === SaleType.REFILL && 
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: { reason: 'Sale edit', updatedFields: Object.keys(validatedData) }
        }
      });
...
export async function DELETE(
  ...
  try {
    ...
    // Validate user permissions
    const permissionCheck = BusinessValidator.validateUserPermission(role, 'DELETE_SALE');
    if (!permissionCheck.isValid) {
      return NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 });
    }
