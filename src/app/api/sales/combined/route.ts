  ...
  try {
    ...
    if (totalValue <= 0) {
      return NextResponse.json({
        error: 'Total sale value must be greater than 0. Please check quantities and prices.'
      }, { status: 400 });
    }
    ...
    const totalCylinderDeposits = Object.values(validatedData.cylinderDeposits || {}).reduce((sum, count) => sum + count, 0);
    ...
    const productIds = validatedData.saleItems.map(item => item.productId).filter(id => id && id.trim() !== '');
    ...
    console.log('Found products:', products.map(p => ({ id: p.id, name: p.name, isActive: p.isActive })));
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const item of validatedData.saleItems) {
        ...
        const itemDiscountPortion = (totalValue > 0 && totalDiscount > 0) ? (itemTotal / totalValue) * totalDiscount : 0;
        const itemNetValue = itemTotal - itemDiscountPortion;
        const itemCashDeposited = (netValue > 0 && itemNetValue > 0) ? (itemNetValue / netValue) * validatedData.cashDeposited : 0;
        ...
        if (item.packageSale > 0) {
          const packageNet = packageValue - (itemTotal > 0 ? (packageValue / itemTotal) * itemDiscountPortion : 0);
          const packageCashDeposited = (netValue > 0 && packageNet > 0) ? (packageNet / netValue) * validatedData.cashDeposited : 0;
          ...
          packageSale = await tx.sale.create({
            ...
            include: {
              ...
              product: { select: { id: true, name: true, size: true, company: { select: { name: true } } } }
            }
          });
        ...
        if (item.refillSale > 0) {
          const refillNet = refillValue - (itemTotal > 0 ? (refillValue / itemTotal) * itemDiscountPortion : 0);
          const refillCashDeposited = (netValue > 0 && refillNet > 0) ? (refillNet / netValue) * validatedData.cashDeposited : 0;
        ...
        validatedData.saleItems.forEach(item => {
          if (item.refillSale > 0) {
            ...
            if (product) {
              ...
              refillBySize.set(size, (refillBySize.get(size) || 0) + item.refillSale);
            }
          }
        });
