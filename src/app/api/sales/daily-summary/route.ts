...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('All recent sales for tenant:', allSales.map(s => ({
      id: s.id,
      saleDate: s.saleDate,
    ...
    console.log('Sales found for date filter:', sales.map(s => ({
      id: s.id,
      saleDate: s.saleDate,
    ...
    const overallSummary = {
      ...
      totalPackageSales: dailySummary.reduce((sum: number, item: any) => sum + item.packageSalesQty, 0),
      totalRefillSales: dailySummary.reduce((sum: number, item: any) => sum + item.refillSalesQty, 0),
      totalSalesValue: dailySummary.reduce((sum: number, item: any) => sum + item.totalSalesValue, 0),
      totalDiscount: dailySummary.reduce((sum: number, item: any) => sum + item.totalDiscount, 0),
      totalCashDeposited: dailySummary.reduce((sum: number, item: any) => sum + item.totalCashDeposited, 0),
      totalCylinderDeposited: dailySummary.reduce((sum: number, item: any) => sum + item.totalCylinderDeposited, 0),
