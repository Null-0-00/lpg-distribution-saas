      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 p-4">
            ...
            <div className="text-xs text-blue-600">cylinders/hour</div>
            <div
              className={`mt-1 text-xs ${getTrendColor(metricsData.salesVelocity.trend)}`}
            >
              {metricsData.salesVelocity.trend === 'up' ? '+' : ''}
              {metricsData.salesVelocity.changePercent.toFixed(1)}% from last
              hour
            </div>
          </div>
          ...
          <div className="rounded-lg bg-gradient-to-br from-green-50 to-green-100 p-4">
            ...
            <div
              className={`mt-1 text-xs ${getTrendColor(metricsData.revenueRate.trend)}`}
            >
          ...
          <div className="rounded-lg bg-gradient-to-br from-purple-50 to-purple-100 p-4">
            ...
            <div className="text-xs text-purple-600">target achievement</div>
            <div
              className={`mt-1 text-xs ${getTrendColor(metricsData.driverEfficiency.trend)}`}
            >
              {metricsData.driverEfficiency.trend === 'up' ? '+' : ''}
              {metricsData.driverEfficiency.changePercent.toFixed(1)}% from
              yesterday
            </div>
          </div>
          ...
          <div className="rounded-lg bg-gradient-to-br from-orange-50 to-orange-100 p-4">
            ...
            <div className="text-xs text-orange-600">weekly rate</div>
            <div
              className={`mt-1 text-xs ${getTrendColor(metricsData.stockTurnover.trend)}`}
            >
              {metricsData.stockTurnover.trend === 'up' ? '+' : ''}
              {metricsData.stockTurnover.changePercent.toFixed(1)}% from last
              week
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 border-t border-gray-200 pt-4">
          <div className="flex items-center justify-between text-xs text-gray-500">
            ...
            <span>Next refresh in {30 - (new Date().getSeconds() % 30)}s</span>
          </div>
        </div>
      </CardContent>
    </Card>
