...
import { Loader2, AlertTriangle, Plus, Minus, Package, RefreshCw } from 'lucide-react';
...
const combinedSaleFormSchema = z.object({
  ...
  saleItems: z.array(z.object({
    ...
    message: "Each item must have quantity > 0 and corresponding price > 0"
  })).min(1, 'At least one sale item is required'),
  paymentType: z.nativeEnum(PaymentType),
  discount: z.number().min(0, 'Discount cannot be negative'),
  cashDeposited: z.number().min(0, 'Cash deposited cannot be negative'),
  cylinderDeposits: z.record(z.string(), z.number().int().min(0, 'Cylinder deposits cannot be negative')).optional(),
...
export function CombinedSaleForm({ onSubmit, onCancel, loading = false, initialData }: CombinedSaleFormProps) {
    ...
    saleItems.forEach(item => {
      ...
      if (item.refillSale > 0 && item.productId) {
        ...
        if (product && product.size) {
          refillQuantitiesBySize[product.size] = (refillQuantitiesBySize[product.size] || 0) + item.refillSale;
        }
      }
    });
  ...
  const removeSaleItem = (index: number) => {
    if (saleItems.length > 1) {
      setValue('saleItems', saleItems.filter((_, i) => i !== index));
    }
  };
  ...
  const handleFormSubmit = async (data: z.infer<typeof combinedSaleFormSchema>) => {
    try {
      await onSubmit(data);
      reset();
    } catch (submitError) {
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <Label className="text-lg font-semibold text-gray-900 dark:text-white">Sale Items</Label>
          <Button
            type="button"
            variant="outline"
            size="sm"
        ...
        {saleItems.map((item, index) => {
          ...
          return (
            <div key={index} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg space-y-4 bg-gray-50 dark:bg-gray-800">
              <div className="flex items-center justify-between">
              ...
              <div className="space-y-2">
                ...
                {errors.saleItems?.[index]?.productId && (
                  <p className="text-sm text-red-600">{errors.saleItems[index]?.productId?.message}</p>
                )}
              </div>
              ...
              {product?.inventory?.isLowStock && (
                <Alert className="border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20">
                  ...
                  <AlertDescription className="text-orange-800 dark:text-orange-200">
                    Warning: This product is running low on stock ({product.inventory.fullCylinders} cylinders remaining).
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    max="1000"
                    {...register(`saleItems.${index}.packageSale`, { valueAsNumber: true })}
                  />
                  {errors.saleItems?.[index]?.packageSale && (
                    <p className="text-sm text-red-600">{errors.saleItems[index]?.packageSale?.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    max="1000"
                    {...register(`saleItems.${index}.refillSale`, { valueAsNumber: true })}
                  />
                  {errors.saleItems?.[index]?.refillSale && (
                    <p className="text-sm text-red-600">{errors.saleItems[index]?.refillSale?.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    {...register(`saleItems.${index}.packagePrice`, { valueAsNumber: true })}
                  />
                  {errors.saleItems?.[index]?.packagePrice && (
                    <p className="text-sm text-red-600">{errors.saleItems[index]?.packagePrice?.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    {...register(`saleItems.${index}.refillPrice`, { valueAsNumber: true })}
                  />
                  {errors.saleItems?.[index]?.refillPrice && (
                    <p className="text-sm text-red-600">{errors.saleItems[index]?.refillPrice?.message}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md font-semibold text-sm text-gray-900 dark:text-white">
                  {formatCurrency((item.packageSale || 0) * (item.packagePrice || 0) + (item.refillSale || 0) * (item.refillPrice || 0))}
                </div>
              </div>
            </div>
          );
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <h4 className="font-semibold mb-2 text-gray-900 dark:text-white">Sale Summary</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-600 dark:text-gray-400">{t('total')} {t('quantity')}:</span>
            <div className="font-semibold text-gray-900 dark:text-white">{totals.totalQuantity}</div>
          </div>
          <div>
            <span className="text-gray-600 dark:text-gray-400">Total Value:</span>
            <div className="font-semibold text-gray-900 dark:text-white">{formatCurrency(totals.totalValue)}</div>
          </div>
          <div>
            <span className="text-gray-600 dark:text-gray-400">Total Discount:</span>
            <div className="font-semibold text-gray-900 dark:text-white">{formatCurrency(totals.totalDiscount)}</div>
          </div>
          <div>
            <span className="text-gray-600 dark:text-gray-400">Net Value:</span>
            <div className="font-semibold text-lg text-gray-900 dark:text-white">{formatCurrency(totals.netValue)}</div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value={PaymentType.MFS}>MFS (Mobile Financial Service)</SelectItem>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {Object.keys(totals.refillQuantitiesBySize).length > 0 && (
            <div className="space-y-4 col-span-1 md:col-span-2">
              <Label className="text-base font-semibold">Cylinder Deposits by Size</Label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(totals.refillQuantitiesBySize).map(([size, maxQuantity]) => (
                  <div key={size} className="space-y-2">
                    <Label htmlFor={`cylinderDeposit-${size}`}>
                      {size} Cylinders Deposited 
                      <span className="text-sm text-gray-500 ml-1">(max: {maxQuantity})</span>
                    </Label>
                    <Input
                      ...
                      onChange={(e) => {
                        const currentDeposits = watchedValues.cylinderDeposits || {};
                    ...
                    {errors.cylinderDeposits?.[size] && (
                      <p className="text-sm text-red-600">{errors.cylinderDeposits[size]?.message}</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      {totals.netValue > (watchedValues.cashDeposited || 0) && (
        <Alert className="border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20">
          ...
          <AlertDescription className="text-yellow-800 dark:text-yellow-200">
            This sale will create a cash receivable of {formatCurrency(totals.netValue - (watchedValues.cashDeposited || 0))}
      ...
      {(() => {
        const totalCylinderDeposits = Object.values(watchedValues.cylinderDeposits || {}).reduce((sum, count) => sum + count, 0);
        const hasCylinderReceivables = totals.totalRefillQuantity > totalCylinderDeposits;
        
        if (totals.totalRefillQuantity > 0 && hasCylinderReceivables) {
          const receivablesBySizeList = Object.entries(totals.refillQuantitiesBySize).map(([size, refillQty]) => {
          ...
          return (
            <Alert className="border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20">
              ...
              <AlertDescription className="text-blue-800 dark:text-blue-200">
                This sale will create cylinder receivables: {receivablesBySizeList.join(', ')}
                {watchedValues.customerName ? ` for customer: ${watchedValues.customerName}` : ' (Customer name recommended for tracking)'}.
              </AlertDescription>
            </Alert>
          );
