...
export const useDriverPerformance = ({
  ...
  const fetchDriverPerformance = useCallback(async () => {
    try {
      ...
      console.log(
        '🚀 Fetching driver performance with params:',
        params.toString()
      );
      const response = await fetch(
        `/api/drivers/performance?${params.toString()}`
      );
      ...
      toast({
        title: 'Error',
        description:
          'Failed to load driver performance data. Please try again.',
        variant: 'destructive',
      });
  ...
  const getTopPerformersByRefillSales = useCallback(
    (limit = 5) => {
      return [...drivers]
  ...
  const getTopPerformersByPackageSales = useCallback(
    (limit = 5) => {
      return [...drivers]
  ...
  const getTopPerformersByRevenue = useCallback(
    (limit = 5) => {
      return [...drivers]
  ...
  const getDriversWithHighReceivables = useCallback(
    (threshold = 10000) => {
      return drivers.filter(
        (driver) => driver.totalCashReceivables > threshold
      );
    },
    [drivers]
  );
