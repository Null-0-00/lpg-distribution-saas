...
export class BusinessValidator {
  ...
  static validateUserPermission(role: string, action: string): { isValid: boolean; message?: string } {
    const permissions = {
      ADMIN: ['CREATE_SALE', 'UPDATE_SALE', 'DELETE_SALE', 'VIEW_REPORTS', 'MANAGE_USERS'],
    ...
    return {
      ...
      message: hasPermission ? undefined : `Role ${role} does not have permission for ${action}`
    };
  }
  ...
  static validateReceivables(data: {
    ...
    try {
      ...
      const newCashReceivables = data.previousCashReceivables + cashReceivablesChange;
      
      // Calculate cylinder receivables change
      const cylinderReceivablesChange = data.driverRefillSales - data.cylinderDeposits;
      const newCylinderReceivables = data.previousCylinderReceivables + cylinderReceivablesChange;
  ...
  static validateTenantAccess(userCompanyId: string, resourceCompanyId: string): boolean {
    return userCompanyId === resourceCompanyId;
  }
  
  // Business rule validation for dates
  static validateSaleDate(saleDate: Date): { isValid: boolean; message?: string } {
    ...
    if (sale.getTime() === today.getTime() || sale.getTime() === yesterday.getTime()) {
      return { isValid: true };
    }
    
    if (sale.getTime() > today.getTime()) {
      return { isValid: false, message: 'Cannot create sales for future dates' };
    }
    
    return { isValid: false, message: 'Can only create sales for today or yesterday' };
  }
}