...
export class CylinderTrackingService {
  ...
  async upsertCylinderTrackingRecord(
    ...
    try {
      ...
      // Validate input data
      const validationErrors = await this.validator.validateCylinderTrackingInput(sanitizedInput);
      ...
      const record = await this.prisma.inventoryRecord.upsert({
        where: {
          tenantId_date_productId_cylinderSizeId: {
            ...
            date: dateOnly,
            productId: sanitizedInput.productId || null,
  ...
  async processSalesEvent(
    ...
    const input: CylinderTrackingInput = {
      ...
      salesData: {
        ...
        eventType: CylinderEventType.SALES,
        eventId: saleData.saleId,
        salesPackageQuantity: saleData.saleType === 'PACKAGE' ? saleData.quantity : 0,
        salesRefillQuantity: saleData.saleType === 'REFILL' ? saleData.quantity : 0,
        salesPackageRevenue: saleData.saleType === 'PACKAGE' ? saleData.totalValue : 0,
        salesRefillRevenue: saleData.saleType === 'REFILL' ? saleData.totalValue : 0,
    ...
    const cashReceivablesChange = saleData.netValue - saleData.cashDeposited - saleData.discount;
    const cylinderReceivablesChange = saleData.saleType === 'REFILL' ? 
      saleData.quantity - saleData.cylindersDeposited : 0;
  ...
  async processShipmentEvent(
    ...
    shipmentData: {
      ...
      cylinderSizeId: string;
      shipmentDate: Date;
      shipmentType: 'INCOMING_FULL' | 'INCOMING_EMPTY' | 'OUTGOING_FULL' | 'OUTGOING_EMPTY';
      quantity: number;
      unitCost?: number;
      totalCost?: number;
      isRefill?: boolean;
    ...
    const input: CylinderTrackingInput = {
      ...
      shipmentData: {
        ...
        eventType: CylinderEventType.SHIPMENT,
        eventId: shipmentData.shipmentId,
        shipmentIncomingFullQuantity: shipmentData.shipmentType === 'INCOMING_FULL' ? 
          shipmentData.quantity : 0,
        shipmentIncomingEmptyQuantity: shipmentData.shipmentType === 'INCOMING_EMPTY' ? 
          shipmentData.quantity : 0,
        shipmentOutgoingFullQuantity: shipmentData.shipmentType === 'OUTGOING_FULL' ? 
          shipmentData.quantity : 0,
        shipmentOutgoingEmptyQuantity: shipmentData.shipmentType === 'OUTGOING_EMPTY' ? 
          shipmentData.quantity : 0,
        shipmentPackageQuantity: (shipmentData.shipmentType === 'INCOMING_FULL' && !shipmentData.isRefill) ? 
          shipmentData.quantity : 0,
        shipmentRefillQuantity: (shipmentData.shipmentType === 'INCOMING_FULL' && shipmentData.isRefill) ? 
          shipmentData.quantity : 0,
        shipmentTotalCost: shipmentData.totalCost || 0,
        packagePurchase: (shipmentData.shipmentType === 'INCOMING_FULL' && !shipmentData.isRefill && 
          shipmentData.status === 'COMPLETED') ? shipmentData.quantity : 0,
        refillPurchase: (shipmentData.shipmentType === 'INCOMING_FULL' && shipmentData.isRefill) ? 
          shipmentData.quantity : 0,
        emptyCylindersBuySell: this.calculateEmptyCylindersBuySell(shipmentData)
      }
    };
  ...
  async getCylinderTrackingAggregation(
    ...
    for (const record of records) {
      ...
      // Aggregate data
      agg.totalOnboardingFullCylinders += record.onboardingFullCylinders;
      agg.totalOnboardingEmptyCylinders += record.onboardingEmptyCylinders;
      agg.totalOnboardingCylinderReceivables += record.onboardingCylinderReceivables;
      agg.totalPackageSales += record.salesPackageQuantity;
      agg.totalRefillSales += record.salesRefillQuantity;
      agg.totalSalesRevenue += record.salesTotalRevenue;
      agg.totalCashDeposited += record.salesCashDeposited;
      agg.totalCylinderDeposited += record.salesCylinderDeposited;
      agg.totalIncomingFull += record.shipmentIncomingFullQuantity;
      agg.totalIncomingEmpty += record.shipmentIncomingEmptyQuantity;
      agg.totalOutgoingFull += record.shipmentOutgoingFullQuantity;
      agg.totalOutgoingEmpty += record.shipmentOutgoingEmptyQuantity;
      agg.totalShipmentCost += record.shipmentTotalCost;
      ...
      agg.currentCashReceivables = record.totalCashReceivables;
      agg.currentCylinderReceivables = record.totalCylinderReceivables;
    }
   ...
   */
  private async prepareTrackingData(input: CylinderTrackingInput): Promise<PrismaCylinderTrackingData> {
    ...
    const data: PrismaCylinderTrackingData = {
      ...
      ...existing,
      // Apply new data based on event type
      ...this.mergeEventData(input, existing)
    };
   ...
   */
  private async getExistingRecord(input: CylinderTrackingInput): Promise<Partial<PrismaCylinderTrackingData>> {
    ...
    // Return existing data without Prisma metadata
    const { id, createdAt, updatedAt, calculatedAt, ...existingData } = existing;
    return existingData;
  }
  ...
  private mergeEventData(
    ...
    if (input.onboardingData) {
      Object.assign(merged, {
        onboardingFullCylinders: input.onboardingData.onboardingFullCylinders ?? existing.onboardingFullCylinders ?? 0,
        onboardingEmptyCylinders: input.onboardingData.onboardingEmptyCylinders ?? existing.onboardingEmptyCylinders ?? 0,
        onboardingTotalCylinders: input.onboardingData.onboardingTotalCylinders ?? existing.onboardingTotalCylinders ?? 0,
        onboardingCylinderReceivables: input.onboardingData.onboardingCylinderReceivables ?? existing.onboardingCylinderReceivables ?? 0,
        onboardingDate: input.onboardingData.onboardingDate ?? existing.onboardingDate
      });
    }
    ...
    if (input.salesData) {
      Object.assign(merged, {
        salesPackageQuantity: (existing.salesPackageQuantity ?? 0) + (input.salesData.salesPackageQuantity ?? 0),
        salesRefillQuantity: (existing.salesRefillQuantity ?? 0) + (input.salesData.salesRefillQuantity ?? 0),
        salesPackageRevenue: (existing.salesPackageRevenue ?? 0) + (input.salesData.salesPackageRevenue ?? 0),
        salesRefillRevenue: (existing.salesRefillRevenue ?? 0) + (input.salesData.salesRefillRevenue ?? 0),
        salesTotalRevenue: (existing.salesTotalRevenue ?? 0) + (input.salesData.salesTotalRevenue ?? 0),
        salesCashDeposited: (existing.salesCashDeposited ?? 0) + (input.salesData.salesCashDeposited ?? 0),
        salesCylinderDeposited: (existing.salesCylinderDeposited ?? 0) + (input.salesData.salesCylinderDeposited ?? 0),
        salesDiscounts: (existing.salesDiscounts ?? 0) + (input.salesData.salesDiscounts ?? 0),
        salesNetRevenue: (existing.salesNetRevenue ?? 0) + (input.salesData.salesNetRevenue ?? 0),
        packageSales: (existing.packageSales ?? 0) + (input.salesData.packageSales ?? 0),
        refillSales: (existing.refillSales ?? 0) + (input.salesData.refillSales ?? 0),
        totalSales: (existing.totalSales ?? 0) + (input.salesData.totalSales ?? 0)
      });
    }
    ...
    if (input.shipmentData) {
      Object.assign(merged, {
        shipmentIncomingFullQuantity: (existing.shipmentIncomingFullQuantity ?? 0) + (input.shipmentData.shipmentIncomingFullQuantity ?? 0),
        shipmentIncomingEmptyQuantity: (existing.shipmentIncomingEmptyQuantity ?? 0) + (input.shipmentData.shipmentIncomingEmptyQuantity ?? 0),
        shipmentOutgoingFullQuantity: (existing.shipmentOutgoingFullQuantity ?? 0) + (input.shipmentData.shipmentOutgoingFullQuantity ?? 0),
        shipmentOutgoingEmptyQuantity: (existing.shipmentOutgoingEmptyQuantity ?? 0) + (input.shipmentData.shipmentOutgoingEmptyQuantity ?? 0),
        shipmentPackageQuantity: (existing.shipmentPackageQuantity ?? 0) + (input.shipmentData.shipmentPackageQuantity ?? 0),
        shipmentRefillQuantity: (existing.shipmentRefillQuantity ?? 0) + (input.shipmentData.shipmentRefillQuantity ?? 0),
        shipmentTotalCost: (existing.shipmentTotalCost ?? 0) + (input.shipmentData.shipmentTotalCost ?? 0),
        packagePurchase: (existing.packagePurchase ?? 0) + (input.shipmentData.packagePurchase ?? 0),
        refillPurchase: (existing.refillPurchase ?? 0) + (input.shipmentData.refillPurchase ?? 0),
        emptyCylindersBuySell: (existing.emptyCylindersBuySell ?? 0) + (input.shipmentData.emptyCylindersBuySell ?? 0)
      });
    }
    ...
    if (input.receivablesData) {
      Object.assign(merged, {
        emptyCylinderReceivables: input.receivablesData.emptyCylinderReceivables ?? existing.emptyCylinderReceivables ?? 0,
        cashReceivablesChange: (existing.cashReceivablesChange ?? 0) + (input.receivablesData.cashReceivablesChange ?? 0),
        cylinderReceivablesChange: (existing.cylinderReceivablesChange ?? 0) + (input.receivablesData.cylinderReceivablesChange ?? 0),
        totalCashReceivables: input.receivablesData.totalCashReceivables ?? existing.totalCashReceivables ?? 0,
        totalCylinderReceivables: input.receivablesData.totalCylinderReceivables ?? existing.totalCylinderReceivables ?? 0
      });
    }
    ...
    if (input.inventoryData) {
      Object.assign(merged, {
        fullCylinders: input.inventoryData.fullCylinders ?? existing.fullCylinders ?? 0,
        emptyCylinders: input.inventoryData.emptyCylinders ?? existing.emptyCylinders ?? 0,
        totalCylinders: input.inventoryData.totalCylinders ?? existing.totalCylinders ?? 0
      });
    }
