...
import { z } from "zod";
import { SaleType, PaymentType, PurchaseType, ShipmentType, UserRole } from "@prisma/client";
...
export const saleValidationSchema = z.object({
  ...
  if (data.saleType === SaleType.REFILL && data.cylindersDeposited > data.quantity) {
    return false;
  }
  return true;
}, {
...
export const productValidationSchema = z.object({
  ...
  fullCylinderWeight: z.number().min(0, "Weight cannot be negative").optional(),
  emptyCylinderWeight: z.number().min(0, "Weight cannot be negative").optional(),
  ...
  message: "Full cylinder weight must be greater than empty cylinder weight",
...
export const assetValidationSchema = z.object({
  ...
  depreciationRate: z.number().min(0).max(100, "Depreciation rate must be between 0-100%").optional(),
});
...
export class BusinessValidator {
  ...
  static validateSale(saleData: {
    ...
    if (saleData.paymentType === PaymentType.CASH && saleData.cashDeposited === 0) {
      errors.push("Cash payment requires cash deposit");
    }

    if (saleData.paymentType === PaymentType.CYLINDER_CREDIT && saleData.saleType !== SaleType.REFILL) {
  ...
  static validateUserPermission(
    ...
    const managerOperations = [
      ...
      'VIEW_INVENTORY', 'VIEW_BASIC_REPORTS'
    ];
    ...
    if (userRole === UserRole.MANAGER && !managerOperations.includes(operation)) {
