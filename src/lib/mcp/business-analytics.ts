...
export class MCPBusinessAnalytics extends SequentialThinkingProcessor {
  ...
  async analyzeDriverPerformance(
    ...
    const totalPackageSales = salesData.reduce((sum, day) => sum + day.packageSales, 0);
    const totalRefillSales = salesData.reduce((sum, day) => sum + day.refillSales, 0);
    const totalRevenue = salesData.reduce((sum, day) => sum + day.revenue, 0);
    const totalCashDeposits = salesData.reduce((sum, day) => sum + day.cashDeposits, 0);
    ...
    const packagePerformanceRatio = avgPackagePerDay / targetMetrics.dailyPackageTarget;
    const refillPerformanceRatio = avgRefillPerDay / targetMetrics.dailyRefillTarget;
    const revenuePerformanceRatio = avgRevenuePerDay / targetMetrics.dailyRevenueTarget;
    ...
    const strongestArea = this.identifyStrongestPerformanceArea(packagePerformanceRatio, refillPerformanceRatio, revenuePerformanceRatio);
    const weakestArea = this.identifyWeakestPerformanceArea(packagePerformanceRatio, refillPerformanceRatio, revenuePerformanceRatio);
    ...
    const recommendations = this.generatePerformanceRecommendations(packagePerformanceRatio, refillPerformanceRatio, revenuePerformanceRatio, cashCollectionRatio);
    this.addThought({
    ...
    const overallRating = this.calculateOverallRating(packagePerformanceRatio, refillPerformanceRatio, revenuePerformanceRatio, consistencyScore);
    this.addThought({
    ...
    const result = {
      ...
      performance: {
        totals: { totalPackageSales, totalRefillSales, totalRevenue, totalCashDeposits },
        averages: { avgPackagePerDay, avgRefillPerDay, avgRevenuePerDay },
        ratios: { packagePerformanceRatio, refillPerformanceRatio, revenuePerformanceRatio, cashCollectionRatio },
  ...
  private calculateConsistencyScore(salesData: Array<{ revenue: number }>): number {
  ...
  private identifyStrongestPerformanceArea(packageRatio: number, refillRatio: number, revenueRatio: number): string {
    const ratios = { Package: packageRatio, Refill: refillRatio, Revenue: revenueRatio };
    return Object.entries(ratios).reduce((a, b) => ratios[a[0]] > ratios[b[0]] ? a : b)[0];
  }

  private identifyWeakestPerformanceArea(packageRatio: number, refillRatio: number, revenueRatio: number): string {
    const ratios = { Package: packageRatio, Refill: refillRatio, Revenue: revenueRatio };
    return Object.entries(ratios).reduce((a, b) => ratios[a[0]] < ratios[b[0]] ? a : b)[0];
  }

  private generatePerformanceRecommendations(packageRatio: number, refillRatio: number, revenueRatio: number, cashRatio: number): string[] {
    const recommendations: string[] = [];
    
    if (packageRatio < 0.8) recommendations.push('Focus on increasing package sales through customer acquisition');
    if (refillRatio < 0.8) recommendations.push('Improve refill sales by strengthening customer relationships');
    if (revenueRatio < 0.8) recommendations.push('Review pricing strategy and upselling opportunities');
    if (cashRatio < 0.7) recommendations.push('Improve cash collection processes and payment terms');
    
    return recommendations;
  }

  private analyzeTrends(salesData: Array<{ date: Date; revenue: number }>): { direction: string; strength: string } {
    if (salesData.length < 3) return { direction: 'insufficient data', strength: 'none' };
  ...
  private calculateOverallRating(packageRatio: number, refillRatio: number, revenueRatio: number, consistency: number): number {
