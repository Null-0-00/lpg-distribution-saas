...
export function demonstrateFifoCalculation(): FifoTestResult {
  ...
  sales.forEach((sale, saleIndex) => {
    ...
    if (saleQuantityRemaining > 0) {
      calculation.push(`   ⚠️ WARNING: ${saleQuantityRemaining} units could not be allocated (insufficient inventory)`);
    }
  });
  ...
  calculation.push(`   Average Buying Price per Unit: ৳${averageBuyingPrice.toFixed(2)}`);
  calculation.push(`   Remaining Inventory Value: ৳${remainingInventoryValue}`);
  calculation.push(`   Remaining Inventory Quantity: ${inventoryBatches.reduce((sum, batch) => sum + batch.remainingQuantity, 0)} units`);
...
export function demonstrateWhyFifoMatters(): void {
  ...
  shipments.forEach(s => console.log(`   ${s.date}: ${s.qty} units @ ৳${s.cost}/unit`));
  ...
  sales.forEach((sale, saleIndex) => {
    ...
    for (let batch of batches) {
      ...
      console.log(`     - ${take} units from ${batch.date} batch @ ৳${batch.cost} = ৳${cost}`);
  ...
  console.log(`   Average Buying Price (FIFO): ৳${avgBuyingPrice.toFixed(2)}/unit`);

  // Compare with simple average
  const simpleAvg = shipments.reduce((sum, s) => sum + s.cost, 0) / shipments.length;
  console.log(`   Simple Average of All Purchases: ৳${simpleAvg.toFixed(2)}/unit`);
  console.log(`   Difference: ৳${Math.abs(avgBuyingPrice - simpleAvg).toFixed(2)}/unit`);
