...
export const calculateBudgetUtilization = (spent: number, budget: number | null): number | null => {
...
export const getRemainingBudget = (spent: number, budget: number | null): number | null => {
  if (!budget || budget <= 0) return null;
  return Math.max(0, budget - spent);
};

export const sortExpensesByDate = (expenses: Expense[], direction: 'asc' | 'desc' = 'desc'): Expense[] => {
...
export const sortExpensesByAmount = (expenses: Expense[], direction: 'asc' | 'desc' = 'desc'): Expense[] => {
  return [...expenses].sort((a, b) => {
    return direction === 'asc' ? a.amount - b.amount : b.amount - a.amount;
  });
};

export const filterExpensesByStatus = (expenses: Expense[], status: 'all' | 'pending' | 'approved'): Expense[] => {
...
export const filterExpensesByCategory = (expenses: Expense[], categoryId: string): Expense[] => {
  if (!categoryId) return expenses;
  return expenses.filter(expense => expense.category.id === categoryId);
};

export const filterExpensesByDateRange = (expenses: Expense[], dateFrom: string, dateTo: string): Expense[] => {
...
export const validateExpenseForm = (formData: any): { isValid: boolean; errors: Record<string, string> } => {
