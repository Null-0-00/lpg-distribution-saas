...
export function validateSaleEntry(data: {
  ...
  if (data.saleType === "REFILL" && data.cylindersDeposited > data.quantity) {
    errors.push("Cylinders deposited cannot exceed sale quantity for refill sales");
  }
...
export function validateInventoryCalculation(data: {
  ...
  const calculatedFullCylinders = 
    data.yesterdayFull + data.packagePurchase + data.refillPurchase - data.totalSales;
  
  if (calculatedFullCylinders < 0) {
    errors.push("Calculated full cylinders would be negative - check your data");
  }
