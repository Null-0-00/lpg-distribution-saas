  ...
  try {
    ...
    productsBySize.forEach((sizeProducts, size) => {
      ...
      sizeProducts.forEach((product, index) => {
        console.log(`   ${index + 1}. ${product.company.name} - ${product.name} (${product.currentPrice.toFixed(2)} ৳)`);
      });
      
      // Calculate average price for this size
      const avgPrice = sizeProducts.reduce((sum, p) => sum + p.currentPrice, 0) / sizeProducts.length;
      console.log(`   📊 Average Price: ${avgPrice.toFixed(2)} ৳`);
      console.log(`   🏢 Companies: ${sizeProducts.map(p => p.company.name).join(', ')}`);
      console.log('');
    });
    ...
    if (latestInventory) {
      console.log(`📊 Latest Inventory (${latestInventory.date.toISOString().split('T')[0]}):`);
      console.log(`   Total Full Cylinders: ${latestInventory.fullCylinders}`);
      console.log(`   Total Empty Cylinders: ${latestInventory.emptyCylinders}`);
      console.log('');
    }
    ...
    productsBySize.forEach((sizeProducts, size) => {
      ...
      const fullQuantityForSize = Math.floor((latestInventory?.fullCylinders || 0) / productsBySize.size);
      const emptyQuantityForSize = Math.floor((latestInventory?.emptyCylinders || 0) / productsBySize.size);
      ...
      console.log(`   Unit Price: ${avgPrice.toFixed(2)} ৳ (average of ${sizeProducts.map(p => p.company.name).join(', ')})`);
      ...
      console.log(`   Unit Price: ${emptyPrice.toFixed(2)} ৳ (20% of full price)`);
    ...
    console.log(`Total Cylinder Assets: ${(totalFullValue + totalEmptyValue).toFixed(2)} ৳`);
    ...
    Array.from(productsBySize.keys()).sort().forEach((size) => {
      console.log(`• Full Cylinders (${size}) - Combined from all companies`);
      console.log(`• Empty Cylinders (${size}) - Combined from all companies`);
    });
