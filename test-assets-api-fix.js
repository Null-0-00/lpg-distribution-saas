...
async function testAssetsApiFix() {
  ...
  try {
    ...
    const fixedCashReceivables =
      Number(cashReceivablesResult[0]?.totalCashReceivables) || 0;
    console.log(
      `✅ Fixed Cash Receivables: ${fixedCashReceivables.toFixed(2)} ৳`
    );
    ...
    const fixedCylinderReceivables =
      Number(cylinderReceivablesResult[0]?.totalCylinderReceivables) || 0;
    console.log(
      `✅ Fixed Cylinder Receivables: ${fixedCylinderReceivables} cylinders`
    );
    ...
    console.log(
      `✅ Fixed Empty Cylinders: ${fixedEmptyCylinders} (from inventory record)`
    );
    console.log(
      `✅ Fixed Full Cylinders: ${fixedFullCylinders} (from inventory record)`
    );
    console.log(
      `📅 Inventory Date: ${latestInventory?.date.toISOString().split('T')[0]}`
    );
    ...
    console.log(
      `Cylinder Receivables Asset: ${(fixedCylinderReceivables * cylinderReceivableUnitValue).toFixed(2)} ৳ (${fixedCylinderReceivables} × ${cylinderReceivableUnitValue})`
    );
    console.log(
      `Empty Cylinders Asset: ${(fixedEmptyCylinders * emptyCylinderUnitValue).toFixed(2)} ৳ (${fixedEmptyCylinders} × ${emptyCylinderUnitValue})`
    );
    console.log(
      `Full Cylinders Asset: ${(fixedFullCylinders * fullCylinderUnitValue).toFixed(2)} ৳ (${fixedFullCylinders} × ${fullCylinderUnitValue})`
    );

    const totalCurrentAssets =
      fixedCashReceivables +
