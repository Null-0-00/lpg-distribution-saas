...
async function testAssetsPageValues() {
  ...
  try {
    ...
    console.log(
      `❌ WRONG (Current Assets API): ${(wrongCashReceivables._sum.totalCashReceivables || 0).toFixed(2)} ৳`
    );
    ...
    console.log(
      `✅ CORRECT (Latest per driver): ${Number(correctCashReceivables[0].totalCashReceivables).toFixed(2)} ৳`
    );
    ...
    console.log(
      `❌ WRONG (Current Assets API): ${wrongCylinderReceivables._sum.totalCylinderReceivables || 0} cylinders`
    );
    console.log('   (This sums ALL historical records - incorrect!)');
    ...
    console.log(
      `✅ CORRECT (Latest per driver): ${Number(correctCylinderReceivables[0].totalCylinderReceivables)} cylinders`
    );
    console.log('   (This gets latest record per driver - correct!)');
    ...
    const calculatedEmptyCylinders = Math.max(
      0,
      refillSales + emptyCylindersBuySell
    );

    console.log(
      `🔄 Assets API Calculation: ${calculatedEmptyCylinders} empty cylinders`
    );
    console.log(
      `   Formula: max(0, refillSales(${refillSales}) + emptyBuys(${emptyBuys}) - emptySells(${emptySells}))`
    );
    ...
    if (latestInventory) {
      console.log(
        `📊 Latest Inventory Record (${latestInventory.date.toISOString().split('T')[0]}): ${latestInventory.emptyCylinders} empty cylinders`
      );
      console.log(`   Full cylinders: ${latestInventory.fullCylinders}`);

      if (calculatedEmptyCylinders !== latestInventory.emptyCylinders) {
        console.log(
          `⚠️  MISMATCH: Calculated (${calculatedEmptyCylinders}) ≠ Inventory Record (${latestInventory.emptyCylinders})`
        );
      } else {
    ...
    console.log('=====================================');
    console.log(
      `Cash Receivables: ${(wrongCashReceivables._sum.totalCashReceivables || 0).toFixed(2)} ৳ → ${Number(correctCashReceivables[0].totalCashReceivables).toFixed(2)} ৳`
    );
    console.log(
      `Cylinder Receivables: ${wrongCylinderReceivables._sum.totalCylinderReceivables || 0} → ${Number(correctCylinderReceivables[0].totalCylinderReceivables)} cylinders`
    );
    console.log(
      `Empty Cylinders: ${calculatedEmptyCylinders} (needs verification against inventory)`
    );
  } catch (error) {
