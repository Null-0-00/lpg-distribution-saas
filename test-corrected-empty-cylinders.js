...
async function testCorrectedEmptyCylinders() {
  ...
  try {
    ...
    correctedValues.forEach((item) => {
      const fullPrice =
        avgPrices.get(item.size) || (item.size === '35L' ? 1200 : 800);
      ...
      console.log(
        `   Unit Price: ${emptyPrice.toFixed(2)} ৳ (20% of ${fullPrice.toFixed(2)} ৳)`
      );
    ...
    console.log(
      `📊 Total Empty Cylinders Value: ${totalEmptyValue.toFixed(2)} ৳`
    );
    ...
    console.log(
      `💡 Difference: ${difference.toFixed(2)} ৳ (${difference > 0 ? 'Higher' : 'Lower'} with corrected values)`
    );
    ...
    const fullQuantityPerSize = Math.floor(
      (latestInventory?.fullCylinders || 0) / 2
    ...
    ['12L', '35L'].forEach((size) => {
      ...
      console.log(
        `• Full Cylinders (${size}): ${fullQuantityPerSize} × ${fullPrice.toFixed(2)} ৳ = ${(fullQuantityPerSize * fullPrice).toFixed(2)} ৳`
      );
      console.log(
        `• Empty Cylinders (${size}): ${emptyQuantity} × ${emptyPrice.toFixed(2)} ৳ = ${(emptyQuantity * emptyPrice).toFixed(2)} ৳`
      );
    });

    console.log('\n✅ Empty cylinder values are now corrected!');
    console.log(
      '🔧 Assets page will show the accurate quantities: 12L=215, 35L=140'
    );
  } catch (error) {
