...
async function testCylinderAssetsBySize() {
  ...
  try {
    ...
    if (latestInventory) {
      console.log(
        `📊 Latest Inventory Totals (${latestInventory.date.toISOString().split('T')[0]}):`
      );
      console.log(`   Full Cylinders: ${latestInventory.fullCylinders} units`);
      console.log(
        `   Empty Cylinders: ${latestInventory.emptyCylinders} units`
      );
      console.log('');
    }
    ...
    productsBySize.forEach((sizeProducts, size) => {
      ...
      const estimatedFullQuantity =
        Math.floor(latestInventory?.fullCylinders / productsBySize.size) || 0;
      const estimatedEmptyQuantity =
        Math.floor(latestInventory?.emptyCylinders / productsBySize.size) || 0;
      ...
      console.log(
        `   Full Cylinders: ${estimatedFullQuantity} × ${fullCylinderPrice.toFixed(2)} ৳ = ${fullValue.toFixed(2)} ৳`
      );
      console.log(
        `   Empty Cylinders: ${estimatedEmptyQuantity} × ${emptyCylinderPrice.toFixed(2)} ৳ = ${emptyValue.toFixed(2)} ৳`
      );
    ...
    console.log('===========');
    console.log(
      `Total Full Cylinders Value: ${totalExpectedFullValue.toFixed(2)} ৳`
    );
    console.log(
      `Total Empty Cylinders Value: ${totalExpectedEmptyValue.toFixed(2)} ৳`
    );
    console.log(
      `Total Cylinder Assets: ${(totalExpectedFullValue + totalExpectedEmptyValue).toFixed(2)} ৳`
    );
    ...
    console.log(
      `  Full Cylinders: ${latestInventory?.fullCylinders || 0} × 500 ৳ = ${oldFullValue.toFixed(2)} ৳`
    );
    console.log(
      `  Empty Cylinders: ${latestInventory?.emptyCylinders || 0} × 100 ৳ = ${oldEmptyValue.toFixed(2)} ৳`
    );
    ...
    console.log(
      `  Total: ${(totalExpectedFullValue + totalExpectedEmptyValue).toFixed(2)} ৳`
    );
    console.log('');

    const difference =
      totalExpectedFullValue +
      totalExpectedEmptyValue -
      (oldFullValue + oldEmptyValue);
    console.log(
      `💡 Difference: ${difference.toFixed(2)} ৳ (${difference > 0 ? 'Higher' : 'Lower'} with size-based pricing)`
    );
    ...
    console.log(
      '🔧 Each size will have its own unit price based on the product price.'
    );
  } catch (error) {
