...
async function testCylinderReceivablesRemoval() {
  ...
  try {
    ...
    console.log(
      `✅ Empty Cylinders: ${emptyCylinders} units @ ${emptyCylinderUnitValue} ৳ = ${(emptyCylinders * emptyCylinderUnitValue).toFixed(2)} ৳`
    );
    console.log(
      `✅ Full Cylinders: ${fullCylinders} units @ ${fullCylinderUnitValue} ৳ = ${(fullCylinders * fullCylinderUnitValue).toFixed(2)} ৳`
    );
    ...
    const cylinderReceivables =
      Number(cylinderReceivablesResult[0]?.totalCylinderReceivables) || 0;
    console.log(
      `❌ Cylinder Receivables: ${cylinderReceivables} units (NOT included as separate asset)`
    );
    ...
    console.log(
      `Empty Cylinders Asset: ${(emptyCylinders * emptyCylinderUnitValue).toFixed(2)} ৳`
    );
    console.log(
      `Full Cylinders Asset: ${(fullCylinders * fullCylinderUnitValue).toFixed(2)} ৳`
    );
    ...
    console.log('===============');
    console.log(
      '• Empty Cylinders inventory already represents cylinders that are with drivers'
    );
    console.log(
      '• These are the same cylinders that appear in "Cylinder Receivables"'
    );
    console.log(
      '• Including both would be double-counting the same physical assets'
    );
    console.log('• Only Cash Receivables represent actual money owed');
    console.log(
      '• Cylinder inventory represents physical assets (empty/full cylinders)'
    );
