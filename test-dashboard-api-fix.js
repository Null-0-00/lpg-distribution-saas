...
async function testDashboardApiFix() {
  ...
  try {
    ...
    console.log(
      `   Total Cash Receivables: ${Number(result[0].totalCashReceivables).toFixed(2)} ৳\n`
    );
    ...
    driverBreakdown.forEach((driver, index) => {
      ...
      console.log(
        `   ${index + 1}. ${driver.driverName}: ${cashAmount.toFixed(2)} ৳ (${driver.date.toISOString().split('T')[0]})`
      );
    ...
    console.log(
      `✅ API Query Result: ${Number(result[0].totalCashReceivables).toFixed(2)} ৳`
    );
    console.log(
      `✅ Match: ${manualTotal === Number(result[0].totalCashReceivables) ? 'YES' : 'NO'}\n`
    );

    console.log('🎯 Expected Dashboard Behavior:');
    console.log(
      `   "বাকি" box should now show: ${Number(result[0].totalCashReceivables).toFixed(2)} ৳`
    );
    console.log(
      '   This matches the "বিক্রয় নগদ প্রাপ্য" total on /dashboard/receivables page ✅'
    );
  } catch (error) {
