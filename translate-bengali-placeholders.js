...
try {
  ...
  const placeholderRegex = /(\w+):\s*'(\w+_bn)',?\s*\/\/\s*TODO:\s*Add Bengali translation/g;
  const placeholders = [];
  let match;
  ...
  const englishTranslationsMatch = content.match(/const englishTranslations: Translations = \{([\s\S]*?)\};/);
  ...
  for (const placeholder of placeholders) {
    ...
    const englishMatch = englishContent.match(new RegExp(`${placeholder.property}:\\s*'([^']*)'`));
    const englishValue = englishMatch ? englishMatch[1] : '';
    
    // Generate Bengali translation
    const bengaliTranslation = generateBengaliTranslation(placeholder.property, englishValue);
    ...
    if (translatedCount % 100 === 0) {
      console.log(`Translated ${translatedCount}/${placeholders.length} properties...`);
    }
  }
  ...
  console.log(`✅ Successfully translated ${translatedCount} Bengali properties!`);
