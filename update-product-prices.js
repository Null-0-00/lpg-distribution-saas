...
async function updateProductPrices() {
  ...
  try {
    ...
    products.forEach((product, index) => {
      ...
      console.log(`${index + 1}. ${product.company.name} ${size} - Current Price: ${product.currentPrice.toFixed(2)} ৳`);
    });
    ...
    for (const product of products) {
      ...
      console.log(`✅ Updated ${product.company.name} ${size}: ${product.currentPrice.toFixed(2)} ৳ → ${newPrice.toFixed(2)} ৳`);
    }
    ...
    updatedProducts.forEach((product, index) => {
      const size = product.cylinderSize?.size || product.size || 'Unknown';
      console.log(`${index + 1}. ${product.company.name} ${size} - New Price: ${product.currentPrice.toFixed(2)} ৳`);
    });
