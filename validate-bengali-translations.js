...
try {
  ...
  if (remainingPlaceholders) {
    console.log(`⚠️  Found ${remainingPlaceholders.length} remaining placeholders:`);
  ...
  console.log(`  Property count match: ${englishProps === bengaliProps ? '✅' : '❌'}`);
  
  // Check for common translation patterns
  const bengaliContent = bengaliMatch[1];
  
  // Count different types of translations
  const bengaliScriptCount = (bengaliContent.match(/[\u0980-\u09FF]/g) || []).length;
  const englishInBengaliCount = (bengaliContent.match(/[a-zA-Z]/g) || []).length;
  
  console.log(`\n🔤 Character Analysis:`);
  console.log(`  Bengali script characters: ${bengaliScriptCount}`);
  console.log(`  English characters in Bengali: ${englishInBengaliCount}`);
  console.log(`  Bengali script ratio: ${Math.round((bengaliScriptCount / (bengaliScriptCount + englishInBengaliCount)) * 100)}%`);
  
  // Check for specific translation quality indicators
  const needsTranslationCount = (bengaliContent.match(/বাংলা অনুবাদ প্রয়োজন/g) || []).length;
  ...
  const sampleMatches = bengaliContent.match(/^\s*([a-zA-Z][a-zA-Z0-9_]*)\s*:\s*'([^']*)',?/gm);
  if (sampleMatches) {
    sampleMatches.slice(0, 10).forEach(match => {
      const [, property, value] = match.match(/^\s*([a-zA-Z][a-zA-Z0-9_]*)\s*:\s*'([^']*)',?/);
  ...
  if (remainingPlaceholders && remainingPlaceholders.length > 0) {
    console.log(`❌ Translation incomplete - ${remainingPlaceholders.length} placeholders remaining`);
  } else if (needsTranslationCount > 50) {
    console.log(`⚠️  Translation needs improvement - many "needs translation" markers found`);
  } else if (bengaliScriptCount < englishInBengaliCount) {
    console.log(`⚠️  Translation quality could be improved - more English than Bengali characters`);
  } else {
